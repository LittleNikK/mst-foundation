...
function CardContent({ card }: { card: CardDef }) {
    ...
    return (
      <div className="grid grid-cols-2 gap-3 sm:gap-3.5">
        {[
          ...
          <div key={item.label} className="p-3 sm:p-3.5 rounded-xl bg-slate-50/90 border border-slate-200/90">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[13px] font-bold text-[#0F172A]">{item.label}</span>
              <span className="text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-blue-50 text-[#3B7DD8]">
                {item.status}
              </span>
            </div>
            <p className="text-[11.5px] text-slate-500 font-medium">{item.detail}</p>
          </div>
        ))}
      </div>
    );
      ...
      <div className="space-y-3">
        {[
          ...
          <div key={row.label} className="p-3 rounded-xl bg-slate-50/90 border border-slate-200/90">
            <div className="flex justify-between items-center text-[13px] font-semibold mb-2 text-[#0D1117]">
              <span>{row.label}</span>
              <span className="text-[#3B7DD8] font-bold text-xs bg-blue-50 px-2.5 py-0.5 rounded-full border border-blue-100">
      ...
      <div className="grid grid-cols-4 gap-2 sm:gap-2.5">
        {[
          ...
          >
            ...
            <span className="text-[12px] font-bold text-[#0D1117] leading-tight mb-1">
              {stage.label}
            </span>
            <span className={`text-[10px] sm:text-[10.5px] leading-tight font-medium ${stage.active ? "text-[#9E7D3B]" : "text-slate-500"}`}>
              {stage.status}
            </span>
          </div>
        ))}
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
        {[
          ...
          >
            <div>
              <p className="text-[13px] font-bold text-[#0D1117] group-hover/doc:text-[#3B7DD8] transition-colors">
                {doc.label}
              </p>
    ...
    return (
      <div className="grid grid-cols-2 gap-2.5 sm:gap-3">
        {[
          ...
          >
            <span
              className={`flex h-8 w-8 flex-none items-center justify-center rounded-lg text-[11px] font-extrabold ${
            ...
            <div>
              <p className="text-[12.5px] font-bold text-[#0D1117] leading-tight">{seat.name}</p>
              <p className="text-[11px] text-slate-500 leading-tight mt-1">{seat.sub}</p>
            </div>
          </div>
        ))}
      </div>
  ...
  // Card 06 — dark
  return (
    <div className="rounded-xl border border-sky-500/25 bg-slate-900/90 p-4 sm:p-5 space-y-2.5 text-[13px]">
      {[
        ...
        <div key={key} className="flex items-baseline justify-between border-b border-white/[0.08] pb-2 last:border-0 last:pb-0">
          <span className="text-[#6BA3E8] font-bold text-[13px]">{key}</span>
          <span className="text-slate-200 text-[12.5px] font-medium">{val}</span>
        </div>
      ))}
    </div>
  );
...
function StackCard({
  ...
  return (
    ...
    >
      ...
      >
        <span
          className="text-[11px] uppercase tracking-[0.22em] font-bold tabular-nums select-none"
      ...
      <div className="grid grid-cols-1 md:grid-cols-[230px_1fr] gap-6 md:gap-8 lg:gap-10 px-6 sm:px-8 lg:px-10 py-5 sm:py-7 flex-1 items-center">
        <div className="pt-0">
          <span
            ...
            className="block text-[32px] sm:text-[42px] font-extrabold leading-none tabular-nums select-none mb-1.5"
          >
            {card.number}
          </span>
          <h3
            ...
            className="text-[19px] sm:text-[21px] font-bold tracking-tight leading-tight mb-2"
          >
            {card.title}
          </h3>
          <div
          ...
          <div className="w-full max-w-5xl flex flex-col justify-center -translate-y-6 sm:-translate-y-8 lg:-translate-y-12">
            ...
            <div className="w-full mb-4 sm:mb-6">
              <motion.p
                ...
                className="text-[11px] sm:text-[12px] uppercase tracking-[0.22em] font-semibold text-[#9E7D3B] select-none mb-1.5 sm:mb-2"
              >
              ...
              <motion.h2
                ...
                className="text-2xl sm:text-3xl lg:text-[36px] font-bold leading-[1.15] tracking-[-0.03em] text-[#0D1117]"
              >
              ...
              <motion.p
                ...
                className="mt-1.5 sm:mt-2 text-[13px] sm:text-[14.5px] leading-[1.5] text-[#373E4A] max-w-2xl"
              >
